 // --- Variables  ---
        ...
        async function traerDataEquipo(nombreEquipo, contadorReintentos = 0) {
            ...
            try {
                ...
                // // **VERIFICACIÓN DE LA ESTRUCTURA CORREGIDA: BUSCANDO 'content'**
                // if (!Array.isArray(data.content)) {
                //     console.error(`Estructura de API inesperada para ${nombreEquipo}: Falta 'content' array. Contenido completo de la respuesta:`, data);
                //     throw new Error(`Estructura de API inesperada: Falta 'content' array.`);
                // }
                
                const digimonList = data.content; 
                
                // Filtrar porque solo queremos digimon con imagen
                const filteredData = digimonList.filter(d => d.image && d.name);
        ...
        function jugarDeNuevo() {
            ...
            juegoActivo = true;
            estadoJugador.textContent = "¡Mazos listos! Elegí una carta oculta para empezar la Ronda 1.";
            estadoRival.textContent = "Esperando tu movimiento...";
        }
        ...
        async function arrancarElJuego() {
            ...
            juegoActivo = true;
            estadoJugador.textContent = "Listo el pollo! Elegí una carta para empezar la Ronda 1.";
            estadoRival.textContent = "Esperando tu movimiento...";
